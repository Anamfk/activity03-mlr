...
```{r pairs-plot}
hfi_2016 %>% 
  select(pf_score, pf_expression_control, pf_ss_women_missing) %>% 
  ggpairs()
```
...
mlr_mod <- lm_spec %>% 
fit(pf_score ~ pf_expression_control + pf_ss_women_missing, data = hfi_2016)
...
$$
\hat{y} = 2.63 + 0.50 \times \text{pf\\_expression\\_control} + 0.21 \times \text{pf\\_ss\\_women\\_missing}
$$


The y-intercept is `r round(tidy(mlr_mod)$estimate[1], 2)`

$$
\hat{y} = `r tidy(mlr_mod)$estimate[1]` + 0.50 \times \text{pf\\_expression\\_control} + 0.21 \times \text{pf\\_ss\\_women\\_missing}
$$
...
To help with interoperatibility, we will focus on qualitative predictor variables with only two levels.
...
The new variable created is named west_atlantic.


We can identify the new variable because it is specified on the left side of the = sign in the mutate function (west_atlantic = ...).

The values of the new variable west_atlantic are determined by the if_else function.
If the region variable is in the set {"North America", "Latin America & the Caribbean"}, then the value of west_atlantic is set to "No".
Otherwise, if the region variable is not in the specified set, the value of west_atlantic is set to "Yes".
...
Specifically, if you notice that the response variables are highly correlated (co linear), including both does not necessarily add much value as they are essentially saying the same thing.
